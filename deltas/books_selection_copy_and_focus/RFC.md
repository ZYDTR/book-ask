...
## 2026-09-21 微信剪贴板格式兼容修复

用户报告划词不弹出。当前安装二进制仍为 `6e42b802…`，最近纸感两行改动只在设计预览；故障日志显示三次真实划选均在复制前因 `snapshot_incomplete` 退出。只读探针确认 AppKit 列出但拒读两个带下划线的 Qt/微信格式；公开 ApplicationServices `PasteboardCopyItemFlavorData` 能完整读取。证据见 `evidence/books_selection_copy_and_focus/run_20260921_clipboard_compat/before.json`；未存剪贴板内容。

修复限定在剪贴板兼容：保留普通 AppKit 路径，仅在读取失败时按同一项目索引经公开 Pasteboard API 读取；不得用全局首个匹配格式替代多项目数据。使用兼容读取的快照通过同一公开接口按原项目/格式顺序恢复，所有写入错误都必须上报。复制前继续验证 changeCount；任何格式仍不可读时仍拒绝，不丢弃富文本、不清空用户剪贴板、不退回不可靠 AX 文字。日志补读取后端、失败项目与格式名/系统状态码，仅元数据。

回归使用隔离命名剪贴板与合成 Qt/微信格式，覆盖多项目中相同格式的不同字节、完整恢复、既有并发与取消保护；当前用户剪贴板只允许只读核验。构建安装前保留旧包；最终真实 Books 划词与模型回答另行验收。此次不改 UI、预览、模型或权限配置。
