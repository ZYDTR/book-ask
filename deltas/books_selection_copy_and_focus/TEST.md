...
## 2026-09-21 剪贴板格式兼容修复（当前）

- 故障定位：12:03 的三次用户划词均到达 `copy_capture_scheduled`，在 `snapshot_incomplete` 退出，`issueMethod=none`；辅助功能正常。此前纸感两行修改只在设计预览，安装版仍为 `6e42b802…`。
- 只读探针：原剪贴板第一个项目中的 `com.trolltech.anymime.WeChat_RichEdit_Format` / `com.trolltech.anymime.QQ_Unicode_RichEdit_Format` 被 AppKit 判为非法 UTI；公开 Pasteboard API 可读取完整字节。读取前后 changeCount 均为 1467，未写入、清空或记录剪贴板内容。
- 回归先 FAIL 后 PASS：新增真实命名剪贴板、合成 Qt/微信格式、多项目同名格式不同字节测试，旧代码在快照阶段失败；新代码通过完整项目/类型/字节/顺序恢复和普通文本可用性检查。既有竞态、取消、超时、拒绝不完整备份测试继续通过。
- `zsh scripts/test.sh` 完整通过，退出码 0。日志中的四条预期 FAIL 来自显式运行旧行为的负向回归，脚本已确认旧行为被捕获；不代表当前版本失败。构建通过。证据：[本轮目录](../../evidence/books_selection_copy_and_focus/run_20260921_clipboard_compat/)。
- 已备份旧应用 zip，安装新版 SHA-256 `5533f4c56157ce7db157e9ac47afa946452e778ca31db8100f3d1eac53d65c74`。通过系统设置移除旧签名条目并重新添加相同路径，未扩大权限；日志确认 `accessibilityTrusted=true`，随后带有效权限重启。
- 真实端到端尚待验收：工具已在原书双击选中可见 `bulldozer`，但该自动控制操作未产生全局 `books_mouse_event`，因此尚不能据此验证自动复制、弹窗和模型回答。已请用户做一次真实鼠标划选。普通/全屏完整回归未覆盖，不以离线测试替代。
- 本轮结束时只读比对 PASS：诊断、安装与授权刷新前后，原剪贴板全部项目/类型/字节完全相同，changeCount 仍为 1467（1 项、8 类型）；内存比对进程已退出，没有任何原剪贴板负载落盘。这不等于真实 Books 复制恢复已验收，因本轮工具手势未进入复制事务。此次只改取词兼容与诊断，不改窗口布局、预览设计或模型。

以下保留 2026-09-19 接手时的分项验收历史。当前状态见上方最新日期记录。需求见 [PRD](PRD.md)，实现合同见 [RFC](RFC.md)，唯一执行绑定见 [索引](../INDEX.md)。逐项结果见文末「执行回填」。
